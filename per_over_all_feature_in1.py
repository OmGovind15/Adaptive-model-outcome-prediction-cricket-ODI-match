...
matplotlib.rcParams.update({
    ...
    'axes.titlesize': 25,     # Plot titles
    'axes.labelsize': 25,     # X and Y axis labels
    'xtick.labelsize': 25,    # X-axis tick numbers/labels
    'ytick.labelsize': 25,    # Y-axis tick numbers/labels
...
# --- MODIFICATION: REMOVED select_features_for_model function ---
...
# (create_correlation_matrix_plot, create_comprehensive_plots, save_feature_selection_analysis)
# ... [Paste all 3 functions from final_in1.py here] ...
...
def create_correlation_matrix_plot(df, features, output_dir):
    ...
    sns.heatmap(corr_matrix, annot=True, cmap='RdBu_r', fmt=".2f", 
                ...
                annot_kws={'fontweight': 'bold'},  # ✅ LARGER text in cells
                cbar_kws={'shrink': 0.8})
    ...
    cbar.set_label('Correlation Coefficient', fontweight='bold', labelpad=15)
...
def create_comprehensive_plots(results_df, model_usage, cv_histories, output_dir):
    ...
    ax.set_ylim(0.6, 1.0)
    # ✅ LARGER TEXT
    ax.set_title('Test Accuracies Across Overs', fontweight='bold', pad=25)
    ax.set_xlabel('Over', fontweight='bold', labelpad=15)
    ax.set_ylabel('Test Accuracy', fontweight='bold', labelpad=15)
    ...
    ax.set_title('Best Model Selection Frequency', fontweight='bold', pad=25)
    # Swap X and Y labels
    ax.set_ylabel('Model', fontweight='bold', labelpad=15)
    ax.set_xlabel('Number of Times Selected', fontweight='bold', labelpad=15)
    
    # Change from xticks to yticks
    ax.set_yticks(range(len(model_counts)))
    ax.set_yticklabels(model_counts.index, rotation=0) # No rotation
...
def save_feature_selection_analysis(feature_selection_log, output_dir):
    ...
    if feature_counts:
        ...
        ax.set_ylabel('Features',  fontweight='bold', labelpad=15)
        ax.set_xlabel('Selection Count', fontweight='bold', labelpad=15)
        
        # Change to yticks
        ax.set_yticks(range(len(features)))
        ax.set_yticklabels(features, rotation=0) # No rotation
        ax.tick_params(axis='x', width=2.5, length=10) # Change to axis='x'
...
def main():
    # --- MODIFICATION: New output directory ---
    output_dir = "comprehensive_analysis_innings1_all_features"
    ...
    # Create and save the feature correlation matrix before dropping NA or scaling
    ...
    print(f"\n{'='*100}")
    print("STARTING COMPREHENSIVE MODEL TRAINING AND ANALYSIS (ALL FEATURES)")
    print(f"{'='*100}")
    ...
    for over in tqdm(sorted(train_data['over'].unique()), desc="Training models for each over"):
        ...
        # --- MODIFICATION: Bypass feature selection ---
        nn_selected_features = features
        nn_method = "all_features"
        ...
        for model_name, grid_search in zip(models.keys(), grid_searches):
            ...
            try:
                # --- MODIFICATION: Bypass feature selection ---
                model_selected_features = features
                ...
                print(f"    Training {model_name} with ALL {len(model_selected_features)} features...")
        ...
        # (This section is identical to final_in1.py)
